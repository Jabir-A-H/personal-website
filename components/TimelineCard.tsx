...
export default function TimelineCard({ title, date, role, description, skills, idx, coAuthors, link, linkLabel }: TimelineCardProps) {
  ...
  return (
    ...
    >
      ...
      {description && <div className="text-sm text-neutral-600 leading-relaxed mb-2">{description}</div>}
      {coAuthors && coAuthors.length > 0 && (
        <div className="text-xs text-neutral-500 mb-2">
          <span className="font-medium text-neutral-700">Co-authors:</span> {coAuthors.join(', ')}
        </div>
      )}
      {link && (
